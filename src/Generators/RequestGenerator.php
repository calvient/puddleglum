...
{
	const CONTROL_KEYS = ['sometimes', 'prohibited', 'required', 'nullable', 'confirmed', 'same'];
	...
	private function parseRules(string $property, array|string $rules): ?array
	{
		...
		$isOptional =
			$rules->has('sometimes') || (!$rules->has('present') && !$rules->has('required'));
	...
	{
		return match ($rule) {
			'nullable', 'sometimes', 'present', 'prohibited', 'required', 'same',
			'confirmed' => $rule,
			'exists', 'unique' => $this->resolveColumn($property, $args),
			'accepted', 'accepted_if', 'boolean' => 'boolean',
			'active_url', 'after', 'after_or_equal', 'alpha', 'alpha_dash', 'alpha_num', 'before',
			'before_or_equal', 'current_password', 'date', 'date_equals', 'date_format', 'digits',
			'digits_between', 'email', 'ends_with', 'ip', 'ipv4', 'ipv6', 'json', 'not_regex',
			'password', 'regex', 'starts_with', 'string', 'timezone', 'url', 'uuid' => 'string',
	...
	private function resolveColumn(string $property, ?string $args): ?string
	{
		...
		if (
			...
			!Schema::hasColumn("$prefix$table", $columnName)
		) {
			return null;
		}
	...
	private function mergeArrays(Collection $rules, int $depth): Collection
	{
		...
		[$dotRules, $rules] = $rules
			->map(function (array $value, string $property) {
				...
				$value['types'] = array_values(
					array_filter($value['types'], fn(string $type) => $type !== 'array'),
				);
				$value['is_array'] = null;
				$value['children'] = [];
		...
		foreach ($dotRules as $property => $value) {
			...
			if (
				$rules[$property]['is_array'] !== null &&
				$rules[$property]['is_array'] !== $isArray
			) {
				throw new \RuntimeException(
					'Cannot combine array and object rules for the same property',
				);
			}
